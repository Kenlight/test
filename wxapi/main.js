...
// const API_BASE_URL = 'http://127.0.0.1:8070/wxa'
const API_BASE_URL = 'https://demo.shop.sglsd.com/wxa'


const request = (url, needSubDomain, method, data) => {
  let _url = API_BASE_URL + (needSubDomain ? '/' + CONFIG.subDomain : '') + url
  var SessionId = CONFIG.SessionId
  if (!SessionId) SessionId = wx.getStorageSync('Session-Id')
  return new Promise((resolve, reject) => {
    wx.request({
      ...
      success(request) {
        if (request.header['Set-Cookie']){
          var cookies = ParseSetCookie(request.header['Set-Cookie'])[0];
          if (!CONFIG.SessionId){
            CONFIG.SessionId = cookies['session_id'];
            wx.setStorageSync('Session-Id', cookies['session_id'])
          } else if (CONFIG.SessionId != cookies['session_id']){
            CONFIG.SessionId = cookies['session_id'];
            wx.setStorageSync('Session-Id', cookies['session_id'])
          }
        }
