...
Page({
  ...
  async initRegionDB (pname, cname, dname) {
    ...
    const _cRes = await WXAPI.nextRegion(pObject.id, 'city')
    if (_cRes.code === 0) {
    ...
    const _dRes = await WXAPI.nextRegion(cObject.id, 'area')
    if (_dRes.code === 0) {
  ...
  bindcolumnchange: function(e) {
    ...
    // 追加后面的一级数组
    WXAPI.nextRegion(regionObject.id, column === 0 ? 'city' : 'area').then(res => {
      if (res.code === 0) {
